/**
 * runAuroraScan — Multi-sensor Earth observation fusion (v3.0)
 * 
 * Orchestrator: validates AOI, tiles cells, invokes Python GEE worker for:
 * - Sentinel-2 optical (B4, B8, B11, B12) with cloud filtering + median composite
 * - Sentinel-1 SAR (VV, VH) with proper band extraction
 * - Landsat 8/9 thermal (B10) with median composite
 * - SRTM DEM (elevation, slope)
 * 
 * Per-cell independent sampling. Real data from Earth Engine.
 */
...
// Invoke Python GEE worker for real sensor data
async function invokePythonWorker(cells, commodity, dateRange) {
  const geeServiceAccount = Deno.env.get('AURORA_GEE_SERVICE_ACCOUNT_KEY');
  if (!geeServiceAccount) {
    throw new Error('AURORA_GEE_SERVICE_ACCOUNT_KEY not set');
  }
  ...
  // Write payload to temp file (avoid command line length limits)
  const tempDir = await Deno.makeTempDir();
  const payloadFile = `${tempDir}/payload.json`;
  await Deno.writeTextFile(payloadFile, JSON.stringify(payload));

  try {
    // Invoke Python worker subprocess
    const proc = Deno.run({
      cmd: ['python3', 'geeWorker/gee_sensor_pipeline.py', payloadFile],
      env: {
        ...Deno.env.toObject(),
        AURORA_GEE_SERVICE_ACCOUNT_KEY: geeServiceAccount,
      },
      stdout: 'piped',
      stderr: 'piped',
    });
    ...
    if (!status.success) {
      ...
      console.error('[GEE-WORKER-ERROR]', errorMsg);
      throw new Error(`GEE worker failed: ${errorMsg}`);
    }
...
Deno.serve(async (req) => {
  ...
  let results = null;
  try {
    results = await invokePythonWorker(sampledCells, commodity, dateRange);
  } catch (e) {
    ...
    return Response.json({ error: 'GEE worker failed', scan_id: scanId, details: e.message }, { status: 503 });
  }
